...
func (p *Provider) decodeStaticFields(eval *StaticEvaluator) hcl.Diagnostics {
	...
	if p.ForEach != nil {
		...
		forVal, evalDiags := evalchecks.EvaluateForEachExpression(p.ForEach, forEachRefsFunc, addrs.RootModuleInstance)
		...
		for k, v := range forVal {
			...
			fmt.Printf("DEBUG: Converted key -> Original: %v, Converted: %s\n", k, keyStr)
